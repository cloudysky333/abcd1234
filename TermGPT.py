...
openai.api_key = os.environ.get("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")
...
def gpt_query(model="gpt-3.5-turbo-0301", max_retries=15, sleep_time=2):
...
+ colorama.Fore.YELLOW + '--r [path/to/file.py]' + colorama.Fore.CYAN + '" to open and read some file into context (can do multiple files, and is embedded into some prompt.) Example: "Please change --r [mplexample.py] to be a dark theme"'+ colorama.Style.RESET_ALL)
    ...
    while not GPT_DONE:
        ...
        reply_content = gpt_query(model="gpt-3.5-turbo-0301")
        ...
        message_history.append({"role": "assistant", "content": generated_message})
        message_history.append({"role": "user", "content": "NEXT"})
        print(colorama.Fore.WHITE + colorama.Style.DIM + generated_message + colorama.Style.RESET_ALL)

        if generated_message.lower().startswith("done"):
            GPT_DONE = True
            print(colorama.Fore.YELLOW + colorama.Style.BRIGHT + "Done reached." + colorama.Style.RESET_ALL)
            break
        else:
            commands.append(generated_message)
        time.sleep(1.5)
